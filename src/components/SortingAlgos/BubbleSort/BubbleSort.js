...
export default class BubbleSort extends Component {
  ...
  bubleSort = async (bubbles) => {
    ...
    if (swap) {
      ...
      this.simulation.force('x', d3.forceX((d) => separator += 50).strength(0.3))
        .alphaTarget(0.5)
        .restart()
  ...
  changeBubblePosition = (action) => {
    ...
    if (action === "combine") {
      this.simulation.force('x', d3.forceX(width / 2).strength(forceStrength).x(centre.x))
        .alphaTarget(0.2)
        .restart()
    }
    ...
    return (
      <>
        <Segment textAlign="center" vertical>
          ...
          <div className="btns">
            <Button onClick={(e) => this.changeBubblePosition("split")}>Split</Button>
            <Button onClick={(e) => this.changeBubblePosition("combine")}>Combine</Button>
            <Button primary onClick={(e) => bubbles && this.bubleSort(bubbles)}>Bubble Sort</Button>
          </div>
          <div className="responsive-svg-container" ref={element => (this.bubbleChart = d3.select(element))} />
              ...
              <pre>
                ...
                {arrVis.map((ele, i) => {
                  if (arrVis.length - 1 > i) {
                    ...
                    return <span key={i} className="array-element">{ele.count}</span>
                  }
                })}
                ]
              </pre>
        ...
        <Message
          ...
          list={[
            <a
              key="source_code"
              href="https://github.com/farhankk360/js-visualize/blob/master/src/components/SortingAlgos/BubbleSort/BubbleSort.js"
              target="_blank"
              without="true"
